 ...
 * 发送请求，获取景点信息，并写入文件
 */
    fs.writeFile(__dirname + '/../baiduHotelData.json', '', function (e) {
        if (e) {
            console.log(e);
        }
    })
    city.forEach(function (item) {
        ...
        for (let pageNumber = 0; pageNumber < 100; pageNumber++) {
            sightsOptions = {
                host: 'map.baidu.com',
                path: 'http://map.baidu.com/?qt=con&c=' + item['Id'] + '&wd=%e9%85%92%e5%ba%97&wd2=&pn=' + pageNumber + '&nn=' + pageNumber * 10,
                method: 'GET'
            };
            req = http.request(sightsOptions, function (res) {
                ...
                res.on('end', function () {
                    if (isJSON(responseString) && JSON.parse(responseString).hasOwnProperty('content')) {
                        ...
                        if (resultObject != '') {
                            fs.appendFile(__dirname + '/../baiduHotelData.json', resultObject, function (e) {
                                if (e) {
                                    console.log(e);
                                }
                            })
