    ...
    this.$fetch({
      ...
      if (res.data.error === 1) {
        this.$store.dispatch("City", {
          name: res.data.start,
          X: res.data.startPos.x,
          Y: res.data.startPos.y,
          ID: res.data.startId
        });
        this.$store.dispatch("City", {
          name: res.data.end,
    ...
    getCity(val) {
      ...
      for (let i = 0; i < this.City.length; i++) {
        var point = new BMap.Point(
          (Number(this.City[i].X) / 1000000).toFixed(3),
          (Number(this.City[i].Y) / 1000000).toFixed(3)
        );
      ...
      for (let i = 0; i < cityPath.length; i++) {
        ...
        arrPath.push(
          new BMap.Point(
            (Number(this.City[cityPath[i]].X) / 1000000).toFixed(3),
            (Number(this.City[cityPath[i]].Y) / 1000000).toFixed(3)
          )
        );
      }
      var polyline = new BMap.Polyline(arrPath, {
        strokeColor: "blue",
        strokeWeight: 6,
        strokeOpacity: 0.5
      });
